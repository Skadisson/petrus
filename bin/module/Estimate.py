...
class Estimate:
    ...
    def format_tickets(self, mapped_ticket):
        cached_tickets = self.cache.load_cached_tickets_except(mapped_ticket['Key'], None)
        ...
        try:
            if self.jira_key is not None:
                ...
                if similar_jira_keys is not None:
                    self.cache.add_log_entry(self.__class__.__name__, 'Found ' + str(len(similar_jira_keys)) + ' similar tickets for Ticket "' + self.jira_key + '"')
                else:
